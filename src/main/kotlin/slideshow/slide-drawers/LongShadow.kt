...
class LongShadow(
    ...
    fun load(program: Program) {
        ...
        stone = concrete?.let { file ->
            if (!file.isFile) { println("long shadow: no concrete at ${file.path}"); null }
    ...
    fun plate(file: File): Plate? = plates.getOrPut(file.path) {
        if (!file.isFile) null
        else loadPlate(file, scatter).also { println("long shadow: ${file.path}, ${it.pieces} blocks") }
    }
     ...
     */
    fun draw(drawer: Drawer, bounds: Rectangle, text: String, frame: Int, plate: Plate? = null) {
        ...
        drawer.isolated {
            ...
            drawer.isolatedWithTarget(mask) {
                ...
                if (field) towers(drawer, fieldFor(drawer, text, plate), frame - frames(titleLastStart(text, plate)), plate)
                if (plate != null) drawPlate(drawer, plate, frame)
                else plan(drawer, text, frame) { h -> ColorRGBa(h, 1.0, 1.0, 1.0) }
            }
     ...
     * two ways.
     */
    private fun plan(drawer: Drawer, text: String, frame: Int, paint: (Double) -> ColorRGBa) {
    ...
    private fun fieldFor(drawer: Drawer, text: String, plate: Plate?): List<Tower> =
        fields.getOrPut(if (plate != null) "plate:${plate.labels}" else "text:$text") {
            ...
            println("long shadow: ${placed.size} towers on a ${across}x$down grid round the title, of ${usable.size} of the title's pieces")
            placed
        }
    ...
    /** The field into the plan, each tower at its own rise. */
    private fun towers(drawer: Drawer, towers: List<Tower>, frame: Int, plate: Plate?) {
        ...
        for (t in towers) {
            ...
            if (shape != null && square) {
                ...
                drawer.fill = ColorRGBa(t.height * up, 1.0, t.tone, 1.0)
...
private fun loadPlate(file: File, scatter: Double): LongShadow.Plate {
    ...
    return LongShadow.Plate(labels, n, bounds, shapes, solidity)
}

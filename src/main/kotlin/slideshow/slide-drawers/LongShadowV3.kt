...
/**
 * Long shadow type, v3 — a copy of [LongShadowV2] to take somewhere new, so the sketch
 * `LongShadowType_v3` can change freely without touching v1, v2 or the show's chapter card.
 *
...
class LongShadowV3(
    ...
    /**
     * `fieldOrder = "reveal"`: the frame is first filled with elements alone, one to every cell of
     * the window's grid, the title under them included — all of them standing within [revealFill]
     * seconds. From [revealAt], the elements over the title go down one by one, from the title's
     * middle outward across [revealSpread] with [revealScatter] of a shuffle, each sinking over
     * [revealSink]; the title's blocks click up where the last element over them has gone. The
     * elements are v1's field packed over the whole frame, title included — its module grid, its
     * sizes, streets and gardens, heights and tones, all off the `field*` arguments.
     */
    private val revealFill: Double = 2.4,
    private val revealAt: Double = 3.4,
    private val revealSpread: Double = 5.0,
    private val revealSink: Double = 0.6,
    private val revealScatter: Double = 0.3,
    /**
     * Once the last piece of the title has seated, the frame holds for [revealHold] seconds; then the
     * elements left beside the title go down one by one, nearest the title first with
     * [revealScatter] of a shuffle, across [revealLeaveSpread], each sinking slowly into the floor
     * over [revealLeaveTime] — until the title stands alone.
     */
    private val revealHold: Double = 2.0,
    private val revealLeaveSpread: Double = 6.0,
    private val revealLeaveTime: Double = 1.2,
    /**
     * The title's pieces come up one at a time from its middle outward across [revealSpread], each
     * no sooner than the elements over it have gone, and each rising over [revealRise] — so a large
     * element going does not let a whole word up at once. Last of all, once the elements beside it
     * are in the floor, the title itself comes down to [revealFinal] of its height over
     * [revealSettle], and its shadow draws in to a sliver.
     */
     ...
     * here a piece is a letter. The picture is kept under `build/long-shadow-v3/`.
     */
    fun textPlate(drawer: Drawer, text: String): Plate? = plates.getOrPut("text:$text") {
        ...
        val file = File("build/long-shadow-v3/${text.lowercase().replace(Regex("[^a-z0-9]+"), "-").trim('-')}.png")
        file.parentFile.mkdirs()
        rt.colorBuffer(0).saveToFile(file, async = false)
        if (buildOfBlocks) {
            blockPlate(rt.colorBuffer(0), file).also { println("long shadow v3: \"$text\" built of ${it.pieces} blocks") }
        } else {
            rt.destroy()
            loadPlate(file, 0.0).also { println("long shadow v3: \"$text\" set in the face, ${it.pieces} pieces") }
        }
    }
    ...
    fun plate(file: File): Plate? = plates.getOrPut(file.path) {
        if (!file.isFile) null
        else loadPlate(file, scatter).also { println("long shadow v3: ${file.path}, ${it.pieces} blocks") }
    }
    ...
    private fun yardFor(plate: Plate): List<Spot> = yards.getOrPut(plate) {
        ...
        println("long shadow v3: ${n} pieces packed in the yard")
        spots.map { it!! }
    }
    ...
    private class Stack(val centre: Vector2, val w: Double, val h: Double, val round: Boolean, val members: List<Int>)
    ...
    private fun stacksFor(plate: Plate): List<Stack> = stackSets.getOrPut(plate) {
        ...
        val kinds = boxes.indices.groupBy(key).entries
            .sortedWith(compareBy({ -it.key.first * it.key.second }, { it.key.third }, { it.key.first }))
        // The whole window as one grid; the cells the title will stand in kept clear.
        val (cw, ch) = gridCell()
        val cols = gridColumns
        val rows = (HIGH / ch).roundToInt()
        val title = (plate.bounds ?: Rectangle(0.0, 0.0, WIDE, HIGH)).offsetEdges(blockUnit)
        val free = (0 until rows).flatMap { r -> (0 until cols).map { c -> c to r } }
            .filter { (c, r) -> !Rectangle(c * cw, r * ch, cw, ch).intersects(title) }
        // As few blocks a stack as lets the stacks fit the grid: counted, not stated.
        var per = 1
        fun lotsAt(per: Int) = kinds.flatMap { (_, m) -> m.chunked(per) }
        while (lotsAt(per).size > free.size && per < boxes.size) per++
        per = maxOf(per, minOf(stackMax, per))
        val lots = lotsAt(per)
        // Spread evenly over every free cell, reading order, so the whole window carries material.
        val stacks = lots.mapIndexed { k, members ->
            val (c, r) = free[(k.toLong() * free.size / lots.size).toInt()]
            val b = boxes[members.first()]
            Stack(Vector2((c + 0.5) * cw, (r + 0.5) * ch), b.width, b.height, plate.pieceKinds[members.first()] == 1, members)
        }
        println("long shadow v3: ${boxes.size} blocks, ${kinds.size} kinds, a ${cols}x$rows grid with ${free.size} cells clear of the title: ${lots.size} stacks of up to $per")
        stacks
    }
    ...
    private fun stackBuild(drawer: Drawer, plate: Plate, time: Double) {
        ...
        val beat = DoubleArray(n) { buildAt + (plate.pieceOrder.getOrNull(it) ?: 0.0) * buildSpread }
    ...
    // ---- the reveal: a full frame of elements, going down over the title to show it ----------- //

    /** One element of the reveal: its cell, its shape, height and roof, when it stands and when it goes (null: it stays). */
     ...
     * when it has one (an svg's path) and as a block or a circle when it has not (the title set in
    ...
    private val covers = mutableMapOf<String, List<Cover>>()

    private fun blockTitle(plate: Plate): Title = titles.getOrPut("plate:${plate.labels}") {
        Title("plate:${plate.labels}", plate.pieceBoxes, plate.pieceBoxes.map { null }, plate.pieceKinds.map { it == 1 }) { i, cell ->
            plate.pieceBoxes[i].intersects(cell)
        }
    }!!

    /**
     * A drawn title off an svg on the pane's own 1920x1080 artboard, every black path a piece, read
     * once. The artboard's white rectangles are left out, and anything half the artboard's size,
     * the trap `ExtrudedType` records. A piece reaches into a cell when its outline crosses the cell
     * or the cell's middle is inside it — so a diagonal stroke clears only the cells it crosses, not
     * every cell in its box. Null, with a line, when the file is missing or holds nothing.
     */
    private fun svgTitle(file: File): Title? = titles.getOrPut("svg:${file.path}") {
        if (!file.isFile) { println("long shadow v3: no svg at ${file.path}, setting the title in the face"); return@getOrPut null }
            ...
            shapes[i].bounds.intersects(cell) &&
                (outlines[i].any { cell.contains(it) } || shapes[i].contains(cell.center) ||
                    listOf(cell.corner, cell.corner + Vector2(cell.width, 0.0), cell.corner + Vector2(0.0, cell.height),
                        cell.corner + Vector2(cell.width, cell.height)).any { shapes[i].contains(it) })
        }
    }
    ...
    private fun coversFor(drawer: Drawer?, title: Title): List<Cover> = covers.getOrPut(title.key) {
        ...
        // square ones, streets left open and gardens flat on the ground. Plain slabs and circles —
        // no plate — since the title's own pieces are what the elements give way to.
        val towers = fieldFor(drawer, "", null, whole = true)
        fun reaches(box: Rectangle) = title.boxes.indices.any { title.touches(it, box) }
        val over = towers.filter { reaches(it.box) }.toSet()
        val far = over.maxOfOrNull { it.box.center.distanceTo(middle) }?.coerceAtLeast(1.0) ?: 1.0
        // Rank rather than raw distance, so the elements go at an even pace however they are spread.
        val keyed = over.map { it to (1.0 - revealScatter) * it.box.center.distanceTo(middle) / far + revealScatter * random.nextDouble() }
        val rank = keyed.sortedBy { it.second }.mapIndexed { k, (t, _) -> t to k.toDouble() / maxOf(1, keyed.size - 1) }.toMap()
        // Those beside the title leave afterwards nearest it first: ranked by distance to the nearest
        // element that went, with the same share of shuffle.
        val side = towers.filter { it !in over }
        val near = side.map { c -> c to ((1.0 - revealScatter) * (over.minOfOrNull { it.box.center.distanceTo(c.box.center) } ?: 0.0) / WIDE +
                revealScatter * random.nextDouble()) }
        val leave = near.sortedBy { it.second }.mapIndexed { k, (t, _) -> t to k.toDouble() / maxOf(1, near.size - 1) }.toMap()
        val placed = towers.map { t ->
            Cover(
                box = t.box, round = t.round, height = t.height, tone = t.tone,
    ...
    private fun planFor(drawer: Drawer?, title: Title): Plan = plans.getOrPut(title.key) {
        ...
        val clear = clears.getOrPut(title.key) {
            DoubleArray(title.boxes.size) { i ->
                cells.filter { it.sink != null && title.touches(i, it.box) }
                    .maxOfOrNull { it.sink!! + revealSink } ?: revealAt
            }
        }
        // Each piece's own turn: from the title's middle outward, evenly across [revealSpread], and
        // never before the elements over it have gone.
        val start = starts.getOrPut(title.key) {
            val middle = title.bounds.center
            val order = title.boxes.indices.sortedBy { title.boxes[it].center.distanceTo(middle) }
            val beat = DoubleArray(title.boxes.size)
            order.forEachIndexed { k, i -> beat[i] = revealAt + revealSink + revealSpread * k / maxOf(1, order.size - 1) }
    ...
    private fun reveal(drawer: Drawer, title: Title, time: Double) {
        ...
        for (c in cells) {
            ...
            if (c.sink != null) {
                ...
                // Getting less high, eased at both ends, until it is in the floor: nothing fades.
                val t = ((time - leaveFrom - c.leave * revealLeaveSpread) / revealLeaveTime).coerceIn(0.0, 1.0)
                h *= 1.0 - t * t * (3.0 - 2.0 * t)
            }
        ...
        for (i in title.boxes.indices) {
            ...
            val r = ((time - start[i]) / revealRise.coerceAtLeast(0.01)).coerceIn(0.0, 1.0)
...
private fun loadPlate(file: File, scatter: Double): LongShadowV3.Plate {
    val img = ImageIO.read(file)
    val w = img.width
    val h = img.height
    ...
    return LongShadowV3.Plate(labels, n, bounds, shapes, solidity, boxes, readOrder)
}
